...
export const createTaskActivityValidator = [
  body("activity")
    ...
    .isLength({ max: LIMITS.ACTIVITY_MAX })
    .withMessage(`Activity cannot exceed ${LIMITS.ACTIVITY_MAX} characters`)
    .escape(),

  body("taskId")
    .trim()
    .notEmpty()
    .withMessage("Parent task is required")
    .custom(
      (value) =>
  ...
  body("materials")
    ...
    .custom(
      (value) =>
        value.length <= LIMITS.MAX_MATERIALS ||
        (() => {
          throw new Error(
    ...
    .custom((value) => {
      ...
      const isValidStructure = value.every(
        (item) =>
          item.materialId &&
          mongoose.Types.ObjectId.isValid(item.materialId) &&
      ...
      if (!isValidStructure) {
        throw new Error(
          "Invalid materials format. Must be array of { materialId: ID, quantity: Number }"
        );
      }
      return true;
    })
    .custom(async (value, { req }) => {
      ...
      if (materials.some((m) => m.isDeleted))
        throw new Error("One or more materials are deleted");
      if (
        materials.some(
          (m) =>
            m.organization.toString() !== req.user.organization._id.toString()
        )
      ) {
  ...
  body("materials")
    ...
    .custom(async (value, { req }) => {
      ...
      if (materials.length !== new Set(materialIds).size) {
        throw new Error(
          "One or more materials not found or belong to another organization"
        );
      }
      if (materials.some((m) => m.isDeleted))
        throw new Error("One or more materials are deleted");
...
export const taskActivityIdValidator = [
  param("activityId")
    .trim()
    .notEmpty()
    .withMessage("TaskActivity ID is required")
    .custom(
      (value) =>
        mongoose.Types.ObjectId.isValid(value) ||
        (() => {
          throw new Error("Invalid task activity ID");
        })()
    )
    .custom(async (value, { req }) => {
      const { default: TaskActivity } = await import(
