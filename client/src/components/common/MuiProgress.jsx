...
const MuiProgress = forwardRef(
  (
    ...
    if (type === "linear" && showLabel && variant === "determinate") {
      return (
        <Box sx={{ display: "flex", alignItems: "center", width: "100%", ...containerSx }}>
    ...
    if (showLabel && variant === "determinate") {
      return (
        <Box sx={{ position: "relative", display: "inline-flex", ...containerSx }}>
          <CircularProgress
            ref={ref}
            variant={variant}
            value={value}
          ...
          >
            <Typography variant="caption" component="div" color="text.secondary" {...labelProps}>
              {`${Math.round(value)}%`}
            </Typography>
          </Box>
        </Box>
